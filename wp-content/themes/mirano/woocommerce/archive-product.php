	...
	<div class="page-content">
		...
		if( is_shop() ){ ?>
			<div class="shop_header">
				<div class="container">
					<h1><?php esc_html_e('Shop All Products', 'mirano'); ?></h1>
					<?php
					$cargs = array(
					...
					?>
						<ul>
							<li class="active"><a href="<?php echo esc_attr($shop_page_url);?>"><?php esc_html_e('All', 'mirano');?></a></li>
							<?php
							foreach($pcategories as $pcategoy) { ?>
								<li><span>/</span><a href="<?php echo get_term_link($pcategoy->slug, 'product_cat'); ?>"><?php echo esc_html($pcategoy->name); ?></a></li>
							 <?php } ?>
						</ul>
						<?php
					} ?>
					...
					<div id="archive-product" class="col-xs-12 <?php echo 'col-md-'.$shopcolclass; ?>">
						...
						<div class="archive-border <?php echo esc_attr($road_shopclass); if($shopsidebar=='left') {echo ' left-sidebar'; } if($shopsidebar=='right') {echo ' right-sidebar'; } ?>">
							...
							<?php if ( have_posts() ) : ?>
								...
								<div class="toolbar">
									<div class="view-mode">
										<a href="#" class="grid <?php if($road_viewmode=='grid-view'){ echo ' active';} ?>" title="<?php echo esc_attr__( 'Grid', 'mirano' ); ?>"><i class="fa fa-th"></i></a>
										<a href="#" class="list <?php if($road_viewmode=='list-view'){ echo ' active';} ?>" title="<?php echo esc_attr__( 'List', 'mirano' ); ?>"><i class="fa fa-list"></i></a>
									</div>
									<?php
										/**
