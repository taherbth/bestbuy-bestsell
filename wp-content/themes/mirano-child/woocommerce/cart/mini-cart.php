...
<div class="cart-toggler">
	<a href="<?php echo WC()->cart->get_cart_url(); ?>">
		<span class="mini-cart-link">
			<i class="fa fa-shopping-cart"></i><span class="cart-title"><?php esc_html_e('My cart: ', 'mirano');?></span>
		</span>
		<span class="cart-quantity">
			<?php echo sprintf(_n('%d item : ', '%d items : ', $woocommerce->cart->cart_contents_count, 'woocorau'), $woocommerce->cart->cart_contents_count);?>
		</span>
	...
	<div class="mini_cart_inner">
		...
		<?php else: ?>
			<ul class="cart_empty <?php echo esc_attr($args['list_class']); ?>">
				<li><?php esc_html_e( 'You have no items in your shopping cart', 'mirano' ); ?></li>
				<li class="total"><?php esc_html_e( 'Subtotal', 'mirano' ); ?>: <?php echo WC()->cart->get_cart_subtotal(); ?></li>
			</ul>
		<?php endif; ?>

		<?php if ( sizeof( WC()->cart->get_cart() ) > 0 ) : ?>

			<p class="total"><?php esc_html_e( 'Subtotal', 'mirano' ); ?>: <?php echo WC()->cart->get_cart_subtotal(); ?></p>

			<?php do_action( 'woocommerce_widget_shopping_cart_before_buttons' ); ?>

			<p class="buttons">
				<a href="<?php echo WC()->cart->get_checkout_url(); ?>" class="button checkout wc-forward"><?php esc_html_e( 'Checkout', 'mirano' ); ?></a>
				<!--<a href="<?php echo WC()->cart->get_cart_url(); ?>" class="button wc-forward"><?php esc_html_e( 'View Cart', 'mirano' ); ?></a>-->
			</p>
